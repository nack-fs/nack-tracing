...
namespace NackEngine.core.render.textures
{
    public class TestTexture : Texture
    {
        ...
        public TestTexture(float scale, Color color1, Color color2)
                : this(scale, new SolidColor(color1), new SolidColor(color2)) { }

        public Color Value(float u, float v, NVector point)
        {
            var x = MathF.Floor(inverseScale * point.X());
            var y = MathF.Floor(inverseScale * point.Y());
            var z = MathF.Floor(inverseScale * point.Z());
