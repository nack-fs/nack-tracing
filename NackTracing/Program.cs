...
namespace NackTracing
{
    ...
    internal class Program
    {
        ...
        private static void WindowsScene()
        {
            //// World
            HitCollection world = new HitCollection();
            ...
            // ---------- RENDER ------------
            camera.InitializeRender();
            RenderWindow window = 
                new RenderWindow(camera.imageWidth,
                                1080 / 16 * 9,
                                camera.PixelBuffer);
            ...
            Task.Run(() =>
            {
                Console.WriteLine("Renderizando en segundo plano...");
                Stopwatch sw = Stopwatch.StartNew();

                camera.Render(bvhWorld);

                sw.Stop();
                Console.WriteLine($"Render finalizado en: {sw.Elapsed}");
            });
